...
func (h *Applier) Handle(ctx context.Context, cmd ApplyCmd) error {
	...
	if signedCount == 0 {
		...
		signingEvidence := cwecommand.ArchiveSigningEvidence{
			Signer:           cmd.SignerDID,
			CredentialType:   cmd.CredentialType,
			CeremonyID:       ceremony.ID,
			Field:            ceremony.FieldName,
			SignedAt:         signedAt,
			PDFCID:           cid,
			PDFHash:          signedPDFHash,
			CredentialHashes: credentialHashes,
		}
		if err := h.archiveSignedContract(ctx, tx, cmd.DID, cmd.AppliedBy, signingEvidence); err != nil {
			return err
		}
	}
